#!/usr/bin/env bash
# ops 層的 fallback 入口，內容與各服務 repo 根目錄的 pm2-start.sh 一致，
# 差別只在 cwd 由參數給（服務 repo 還沒有自己那份時用，目前只有 notes）。
#   git pull → HEAD 變了才 build → exec next start
# 由 deploy/ecosystem.config.js 呼叫，不要手打。
#
# 用法： start-service.sh <服務 repo 絕對路徑> <port>
#
