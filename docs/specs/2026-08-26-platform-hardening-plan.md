...
- [x] T-Form 新回覆通知 webhook（**非體檢發現**，部長 2026-08-28 凌晨提出，同日完成）
      ...
      但像是這種回報表單正常不會定時檢查，那就可以用 webhook 通知開起來。」
      做法：`/admin/webhooks` 登記（Discord / Google Chat），每份問卷在設定面板勾選，
      預設全關。通知走 `next/server` 的 `after()`（填寫者不必等 10 秒的 webhook）。
      **通知內容由人決定**（部長 2026-08-28 追加）：每份問卷一個開關，預設「只送關鍵資訊」
      （有新回覆／填寫者／後台連結），可改成「連答案一起送」（含附件檔名）。
      預設仍是關、而且開的時候 UI 會把代價講白：通知進的那個群組，成員名單不在 T-Pass 的
      權限模型裡——auth 的 /admin 把某人降回 default 擋得住後台、擋不住群組，卸任畢業也不會自動收權。
      ⚠️ **tpass-appeals 的申訴通知不套用這個開關**（那裡永遠不送內容——申訴的對象很可能
      就在那個頻道裡）。內容超過 3000 字會截斷，因為超過平台上限是整包被退回。
      網址白名單只收 `discord.com` / `chat.googleapis.com`（擋貼錯，也擋 admin 帳號失守時
      把通知導去任意主機）。33 個測試 + 假 auth 登入實測後台兩個頁面（確認 HTML 不洩漏 secret）。
      **還沒驗的一件事**：真的送到一個真實群組——需要一條真的 webhook URL。
      `/admin/webhooks` 的「測試」按鈕走的就是正式通知那支函式，貼上 URL 按一下即可。
