...
> 本文件專為**應用服務開發者**設計：從零建立一個新服務、接上 T-Pass 單一登入與權限控制，直到正式上線。
> 內容自給自足——所有指令皆為原生 `git` / `pnpm` / `ssh` / `psql`，不需要 ops repo，也不需要任何自製工具。
>
> 全文以 `lost`（遺失物）、port `3007` 為範例，實作時請替換為你的服務 id 與 port。

---

## 核心設計原則

T-Pass 架構遵循：**auth 負責發證，服務負責驗證。服務永遠拿不到、也不需要任何私鑰。**

1. **服務只持有公鑰**：透過 JWKS 抓取公鑰，驗章在服務後端完成。auth 故障不影響已登入的使用者。
2. **票證一服務一張**：JWT 帶有特定受眾 `aud = tpass:<service_id>`，用於其他服務一律驗證失敗。
3. **Cookie 限定本服務網域**：採用 Host-only Cookie（不設 `Domain` 屬性），瀏覽器不會跨站傳送。
4. **絕不存於 localStorage**：所有憑證必須由 Server 端透過 `HttpOnly` Cookie 處理。

> [!IMPORTANT]
> 純前端 SPA 無法完成此串接。Cookie 為 `HttpOnly`、驗章須在 server 端進行，因此服務必須具備後端。
> Next.js 的 Route Handler / Server Component 即可作為後端。
...
②  auth 用一個自動送出的表單，把 token POST 到你的 /api/auth/callback
   （token 全程不出現在網址列、不進瀏覽器歷史、不進 Referer）
       │
③  你的 callback 用 auth 公開的公鑰在自己後端驗章 → 驗過才寫進你自己網域的 cookie
       │
...
有兩項資源僅維運成員持有，需先取得：**dev 用的 Google OAuth client**（id + secret，本機執行 auth 用）與**主機 ssh 帳號**（上線時用，絕不寫入任何 repo、commit、PR）。

> [!IMPORTANT]
> dev 的 Google client 與正式站是不同的兩組，dev 僅放行 `*.lvh.me` 的回跳網址。即使外流也不影響正式站，但仍不應貼入 Slack 或提交至版本控制。

<details>
<summary><b>1. 工作目錄</b></summary>
...
├── tpass-registry/      ← 服務註冊表（你 fork 的那份）
├── tpass-auth/          ← 本機發證端
└── tpass-lost/          ← 你的服務
```
...
<details>
<summary><b>2. 服務註冊（唯一的登記步驟）</b></summary>
...
* **auth 的發證白名單**自動包含你（否則使用者會被導去 `/service-error?reason=unknown-service`）
* **portal 大廳的卡片**自動出現（需 `deployed` 已翻成 `true`）
* **部署腳本與 pm2** 自動認得你的目錄與 port

除此之外**不需要改任何其他 repo 的程式碼**——不必碰 portal，也不必碰 auth。

服務 id 同時對應：pm2 程序名 ＝ `TPASS_SERVICE_ID` ＝ JWT 的 `aud` 後綴 ＝ 服務子網域。**id 一經選定即不再更名**（建議短、全小寫）。
...
在 `services.json` 的 `services` 陣列末端加一筆：
...
```jsonc
{
  ...
  "enabled": true,
  "deployed": false,                                  // 主機前置備妥、要上線時才翻 true
  "portal": {                                         // 沒有這塊 = 不進大廳
    "label": "遺失物",
...
> ——你在 `add-lost` 分支上加的那一筆立刻生效，本機登入馬上能測。merge 只影響**正式站**。
>
...
<details>
<summary><b>3. 建立服務 repo</b></summary>

### 說明

服務一律採用 **Next.js 16 + React 19**（與生態系其他服務一致）。repo 名稱請與註冊表那筆的 `dir` 一致——主機的部署腳本是照 `dir` 找目錄的。
...
<details>
<summary><b>1. 設定模組 (src/config/lost.ts)</b></summary>

### 說明

所有設定集中於此，全部透過 env 讀取。**`REQUIRED` 陣列是「env 必填清單」的唯一真相**：服務啟動時靠它報出缺哪些 key，部署時 `deploy.sh` 也會解析它，在 build 前就擋下缺 key 的情況。之後每加一個必填 env，就要加進來。
...
<details>
<summary><b>2. 驗章核心模組 (src/lib/tpass-auth.ts)</b></summary>

### 說明

負責向 auth 取得公鑰（JWKS）並驗證 JWT 的合法性。遵循四大安全鐵則：鎖定 EdDSA 演算法、驗證 Issuer、驗證 Audience、驗證 Expiration。

`createRemoteJWKSet` 內建快取、依 `kid` 選鑰、金鑰輪替時自動重抓——**不要自己手刻**，硬編碼單一把公鑰會在輪替時全面失效。
...
// read 是唯一必看欄位（auth 已經算好 = restriction !== "ban"）。
...
3. **檢查 `audience` = `tpass:<你的id>`**——遺漏此檢查，其他服務的 token 即可用於冒用本服務，服務隔離形同虛設。
...
<details>
<summary><b>3. 發起登入導向 API (Authorization Request)</b></summary>

### 說明

當使用者未登入時，後端將使用者 302 重導向至 auth 的授權端點。

* **HTTP Method**：`GET`
* **Target URL**：`${AUTH_AUTHORIZE_URL}`
* **Query Parameters**：
  * `service`：你的服務 id（例如 `lost`）
  * `redirect_uri`：登入完成後 auth 需將 token POST 回來的端點（例如 `https://lost.lvh.me:3007/api/auth/callback`）
  * `next`：最終目標站內路徑（例如 `/dashboard`）

### 程式碼範例

實作已在〈設定模組〉的 `loginUrlFor()`；頁面端這樣用：
...
authorize 失敗時**不回 JSON，而是把使用者導向 auth 的說明頁**（因為 authorize 一定是瀏覽器的頂層導航）：

| 導向位置 | 原因 |
| --- | --- |
| `/service-error?reason=unknown-service` | 該 id 不在註冊表裡，或 auth 尚未在 merge 後重新部署 |
| `/service-error?reason=invalid-redirect` | `redirect_uri` 須為完整網址，且網域須在生態系範圍內（防 Open Redirect） |
| `/service-error?reason=invalid-next` | `next` 必須是站內路徑（以 `/` 開頭，且不以 `//` 開頭） |
| `/denied?service=<id>` | 該使用者在你的服務被 ban（`read === false`） |

</details>

<details>
<summary><b>4. 接收 Token 端點 (POST /api/auth/callback)</b></summary>

### 說明

auth 認證成功後，會以 HTML Form POST 方式將 JWT 與 `next` 路徑傳送至此 API（因此 token 不會出現於網址、瀏覽器歷史或 Referer）。本端點負責驗章並寫入 **Host-only `HttpOnly` Cookie**。
...
### 程式碼範例 (`src/app/api/auth/callback/route.ts`)
...
<details>
<summary><b>5. 服務登出端點 (POST /api/auth/logout)</b></summary>

### 說明

登出需同步清除兩處：**本服務的 Cookie**（只有本服務清得掉）與 **auth 端的登入態**。後端以 HTML auto-submit 表單重導向至 auth 的登出端點。

* **HTTP Method**：`POST`
* **Target Action**：`${AUTH_LOGOUT_URL}?redirect_uri=${SELF_URL}`

### 程式碼範例 (`src/app/api/auth/logout/route.ts`)
...
>
> 其他服務的 cookie 會保留至各自過期（最長＝auth 端 `JWT_TTL_SECONDS`，建議 45 分鐘）。這是「一服務一張票」設計下的已知取捨：登出不是全生態即時生效，而是「auth 不再發新票 + 舊票自然過期」。

</details>

<details>
<summary><b>6. 頁面與 API 守門機制 (Guards)</b></summary>

### 說明

展示如何在 Server Component、Route Handler 與 Server Action 中檢查登入狀態與權限。

> [!WARNING]
> **每個 route handler 與 server action 都必須各自重新檢查**，不能僅依賴 layout 或頁面層級的攔截——layout 無法阻擋直接呼叫 API 的請求。

### 角色判斷 (`src/config/admin.ts`)
...
> [!NOTE]
> `AUTH_DENIED_URL` 是**選填** env：沒設就用 `AUTH_AUTHORIZE_URL` 的 origin 自動推導 `<origin>/denied`，通常不用另外設。

### 用法 1：頁面防護 (`src/app/admin/layout.tsx`)
...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!isAdmin(session)) return <p>你沒有存取此頁面的權限。</p>;

  return <>{children}</>;
}
```

### 用法 2：Route Handler 防護
...
export async function deleteItem(id: string) {
  await requireAdmin(); // 非管理員直接拋出 ForbiddenError
  // …執行刪除邏輯
}
```
...
<details>
<summary><b>7. 環境變數</b></summary>

### 說明

網址不應寫死於程式碼中——上線時僅需修改 `.env.local`（改為 `*.tschoolsu.org`，**不含 port**），程式碼無需異動。同時將這些 key（以佔位值表示）補入 `.env.example`，供後續維護者參考。
...
> [!IMPORTANT]
> **值含空白時要加引號**（例如逗號後有空格的清單）。`deploy.sh` 會用 shell `source` 匯入這個檔，
> 未加引號的空白會讓該行被當成指令執行，部署中止且錯誤訊息指不到真正原因。

> [!NOTE]
> `lvh.me` 由公共 DNS 直接解析到 `127.0.0.1`，因此本機開發不需修改 `/etc/hosts`。

</details>

---

## 授權與權限模型 (Permissions Claim)

登入（authentication）確認「你是誰」；授權（authorization）決定「你能做什麼」。T-Pass 的授權模型：**auth 在 token 中蓋上 `permissions` 章（per-service 的 role + 管制狀態），各服務讀取它於本地判斷權限。**

服務**不自行維護管理員名單、不查詢資料庫、不使用環境變數**——名單維護在 auth 的 `/admin` 網頁 panel。

<details>
<summary><b>權限資料結構說明</b></summary>
...
* **`restriction`**：`warning`（提醒，仍可使用，呈現方式自訂）與 `ban`（禁止使用，auth 已在 `authorize` 階段攔截，正常情況下你的服務根本收不到 ban 者的新票）。
* **`read`** 是唯一必看欄位：`if (!perm.read) redirect(deniedUrl)` 就是完整的 ban 守門邏輯。
* 權限變更（在 `/admin` 存檔）最長 `JWT_TTL_SECONDS`（建議 45 分鐘）後生效——這是換票成本換來的取捨。**ban 例外，會讓對方的 auth 登入態立刻作廢。**

> [!NOTE]
> 舊版曾有 `groups` claim（`groups.includes("admin")`），已於 2026-07-27 從 auth 簽發邏輯與所有消費端程式碼中移除。**新服務只認 `permissions`**。

</details>

<details>
<summary><b>呈現警告 (WarningBanner)</b></summary>

### 說明

`restriction === "warning"` 的呈現方式由各模組自訂——沒有固定版型，但 `tpass-portal` 有一份可直接抄的範本：`tpass-portal/src/components/WarningBanner.tsx`。
...
<details>
<summary><b>細粒度資源授權 (Resource Level)</b></summary>

### 說明

`role` 判斷的是**角色層級**的權限（是否為管理員）。「能否讀取**某一筆**資料」屬於資源層級的授權，仍應於服務本地依資料的擁有者實作。

比對擁有者請使用 JWT 的 `sub`（Subject ID），**切勿使用 `email`**——`sub` 是跨服務一致的穩定識別碼，`email` 則可能變動。
...
<details>
<summary><b>管理權限：auth 的 /admin panel</b></summary>
...
| 幫某人在**你的服務**設 role 或下 restriction（+ 原因 + 到期） | `/admin/people/[email]` — 每個服務一列 |
| 查誰動了誰的權限 | `/admin/audit` |

panel 本身的存取權：`AUTH_SUPERADMINS`（生態總管，逃生門）或在 `auth` 這個服務本身被設為 admin/moderator。

> [!IMPORTANT]
> 存檔後**不會**立即讓對方的舊票失效（ban 除外）。一般的 role/warning 變更，生效延遲最長＝`JWT_TTL_SECONDS`，對方下次換票才會拿到新權限。panel 會顯示「最晚 HH:MM 生效」。
...
測試登入需同時執行兩個服務：一份 **auth**（發證端）與**你的服務**（驗證端）。兩者皆須為 **HTTPS**，且皆須運行於 `*.lvh.me` 的子網域上。

> [!NOTE]
> **為何不能串接正式站的 auth 進行測試？** auth 設有 `AUTH_ALLOWED_HOST_SUFFIX` 白名單，正式站僅放行 `*.tschoolsu.org` 的回跳網址，本機的 `lost.lvh.me` 必定被擋下。這是防止 Open Redirect 的設計，非缺陷。

<details>
<summary><b>1. 建立本機憑證</b></summary>
...
> [!NOTE]
> `lvh.me` 永遠解析到 `127.0.0.1`，因此不需修改 `/etc/hosts`，同時仍能取得真正的子網域——唯有如此才能正確驗證 cookie 的 host-only 行為（以 `localhost` 測試無法驗證）。

</details>

<details>
<summary><b>2. 啟動本機 auth</b></summary>

### 說明

僅需執行一次。auth 要與 `tpass-registry` **並排**——它會讀 `../tpass-registry/services.json` 決定可以為哪些服務發證，而那份就是你在〈服務註冊〉clone 的 fork，已經含有你的服務。
...
AUTH_SUPERADMINS="<你的學校信箱>"                 # 多個 email 以逗號分隔，整串加引號
```

> [!NOTE]
> **服務白名單不在這裡**。auth 可以為哪些服務發證，完全由 `../tpass-registry/services.json` 決定——就是你在〈服務註冊〉fork 下來的那份。改了那個檔要**重啟 auth** 才生效（清單在啟動時讀進來）。

### 套用資料庫 schema 並啟動

Prisma CLI 只讀 `.env`，所以先把 `.env.local` 匯進環境：
...
<details>
<summary><b>3. 設定 dev 指令</b></summary>

### 說明

以下三個參數缺一不可，將此段貼入你的 repo 的 `package.json`。
...
| `NODE_TLS_REJECT_UNAUTHORIZED=0` | 登入成功後立即被導回登入頁，且無任何錯誤訊息 |

第三項的原理：服務後端需 fetch auth 的公鑰，但 Next（Turbopack）server 端所使用的 fetch（undici）**不讀取 `NODE_EXTRA_CA_CERTS`**，即不信任 mkcert 簽發的憑證。其結果是公鑰擷取失敗 → 驗章失敗 → callback 靜默回傳 401 → 使用者被導回登入頁並反覆循環。

> [!CAUTION]
> `NODE_TLS_REJECT_UNAUTHORIZED=0` **僅能出現在本機的 `dev` 指令中**，不可寫入 `.env`，也不可寫入 `build` / `start`。正式主機使用真憑證，若將其帶上主機，等同關閉所有 TLS 驗證，屬於資安事故。
>
> 另需注意：**auth 本身不可加入此設定**，因其須驗證 Google 的真實憑證。

`packageManager` 使所有開發者的 pnpm 版本自動對齊；`onlyBuiltDependencies` 是 pnpm 10 的 build-script 白名單（pnpm 預設不執行依賴的 postinstall，若未放行，sharp / prisma 的原生二進位檔將無法完整安裝——**此問題在本機不會顯現，僅於部署至主機後才會出現**）。

</details>

<details>
<summary><b>4. 啟動服務與提交前檢查</b></summary>
...
```bash
cd ~/tpass/tpass-auth  && pnpm dev      # 終端機 1：發證端 → https://auth.lvh.me:3000
cd ~/tpass/tpass-lost  && pnpm dev      # 終端機 2：你的服務 → https://lost.lvh.me:3007
```
...
主機上的部署帳號**不具 root 權限**。標記 **[root]** 的步驟需交由有 sudo 的維運成員執行。

<details>
<summary><b>1. 上線前置作業（分工表）</b></summary>
...
| 7 | 你 | 部署 `lost` → `auth` → `portal`（見下一節） |
| 8 | 你 | 瀏覽器真人走一次登入，跑一遍〈整合測試檢查清單〉（把 `lvh.me:3007` 換成正式網域） |

> [!IMPORTANT]
> **第 5 步必須在第 7 步之前。** pm2 的程序清單是從註冊表裡 `deployed: true` 的服務產生的，還是 `false` 的話部署腳本找不到你的服務、起不起來。
>
> 也因此第 5、6 步要連著做完：`deployed` 翻成 `true` 之後、你的 repo 還沒 clone 到主機之前，若有人剛好跑 `./deploy/deploy.sh all` 會在你的服務上失敗（找不到目錄）。這個空窗越短越好。
...
./deploy/deploy.sh auth               # ② 發證白名單納入 lost
...
於 repo 執行 `git revert` 產生新 commit → merge 進 main → 重新執行 `./deploy/deploy.sh lost`，不需特殊機制。build 失敗時舊版程序不受影響、不會停機——`deploy.sh` 採先 build 成功才 reload 的策略。
...
<details>
<summary><b>登入與隔離驗證項目</b></summary>

以下皆可在兩個終端機環境下完成，**不需啟動 portal**。

1. **登入功能**：開 `https://lost.lvh.me:3007` → 自動導向 auth → Google 登入 → 導回本站並顯示自己的名字。

2. **Cookie Host-only 檢查**：DevTools → Application → Cookies → `lost.lvh.me`：有一顆 `tpass_token`，且 **Domain 欄不可有前導點**（應為 `lost.lvh.me` 而非 `.lvh.me`）。有點就是你設了 `Domain`，通行證會外洩給其他服務。

3. **登出功能**：點擊登出 → 本站 cookie 已被完全清除。

4. **SSO 免重複登入**：於 DevTools 只刪掉 `lost.lvh.me` 的 `tpass_token`（**保留 `auth.lvh.me` 的 `tpass_auth_session`**）→ 重新整理 → 應被導去 auth，然後 **auth 不會叫你再點一次 Google，直接把你送回來**。

5. **隔離防護測試（最重要）**：直接於瀏覽器請求下方 URL——注意 `service` 是**別人**（`portal`），但 `redirect_uri` 指向**你的** callback：

   ```
   https://auth.lvh.me:3000/api/auth/authorize?service=portal&redirect_uri=https://lost.lvh.me:3007/api/auth/callback&next=/
   ```

   auth 會簽一張 `aud=tpass:portal` 的票 POST 給你。**驗收標準：你的 callback 必須回應 HTTP 401 Unauthorized。** 若成功登入，代表沒有檢查 `aud`，別人服務的票在你家能用，隔離全毀——回去看〈驗章核心模組〉鐵則 3。

</details>

<details>
<summary><b>授權驗證項目</b></summary>

* [ ] 在 `/admin` 把某人設為你服務的 admin → 他重新登入（或等舊票過期換票）後進得了你自己服務的後台。
* [ ] 名單外的人登入後於你的後台被擋下（顯示禁止存取畫面）。
* [ ] 在 `/admin` 對某人下 `ban` + 原因 → 他重新走一次登入被導去 `<auth>/denied?service=<你的id>`，看得到原因與（若有設）解封時間。
* [ ] 解除 ban 後，他能重新登入使用。
* [ ] 對某人下 `warning` → 他登入後在你的服務看得到警告呈現。
* [ ] `permOf(session)` 對缺資料的情況回傳安全預設值 `{read:true, role:"default"}`，不會誤鎖使用者。
...
| 症狀 | 原因 / 解法 |
| --- | --- |
| 登入完**馬上被踢回登入頁**（本機） | 多半是 dev 指令缺少 `NODE_TLS_REJECT_UNAUTHORIZED=0`，導致後端抓不到 auth 的 JWKS 公鑰。log 裡找 `UNABLE_TO_VERIFY_LEAF_SIGNATURE` |
| 登入完**馬上被踢回登入頁**（主機） | 與 TLS 無關，**絕不要**在主機加 `NODE_TLS_REJECT_UNAUTHORIZED`。查 `iss` / `aud` 是否對應 |
| 瀏覽器說憑證不安全 | 憑證未涵蓋該子網域。重跑 `mkcert`，把 `lost.lvh.me` 加進去；`mkcert -install` 也需重新執行 |
...
| 登入一段時間後失效 | 正常現象，per-service token 壽命＝`JWT_TTL_SECONDS`；重新走一次登入會自動換到新票 |
| 服務一啟動就報「缺少必填環境變數」 | 對照 `src/config/lost.ts` 的 `REQUIRED` 陣列補齊 `.env.local` |
| 部署被擋，說 env 缺 key | 同上，但須補齊的是**主機上**的 `.env.local`。`deploy.sh` 於 build 前即擋下，此為刻意設計 |
| 部署時報 `command not found`，指向一個 email 或網址 | `.env.local` 裡某個值含空白卻沒加引號。`deploy.sh` 用 shell `source` 匯入，未加引號的空白會被當成指令 |
| 首次部署你的服務，pm2 沒起來 / 健康檢查失敗 | 註冊表裡 `deployed` 還是 `false`。pm2 的程序清單由這個欄位產生 |
| 上線了，但 **portal 首頁看不到你的卡片** | 依序確認：① registry 裡你那筆有 `portal` 區塊；② `deployed` 已翻成 `true`；③ portal 在那之後**重新部署過** |
...
* 權限判斷一律讀 JWT 的 `permissions` claim；`groups` 已於 2026-07-27 全面移除。名單維護於**中央**（auth 的 `/admin`），服務**不應**自行維護 allowlist 或 DB 名單。細粒度授權仍應於服務本地實作。
...
<details>
<summary><b>JWT Payload 範例資料結構</b></summary>

`/api/auth/callback` 收到的 token，解開後結構如下：
...
| `name` | `string` | 顯示名稱 |
| `permissions` | `Record<string, PermissionEntry>` | **權限本體**。一般服務 token 只含自己一把 key |
...
<details>
<summary><b>services.json 欄位</b></summary>
